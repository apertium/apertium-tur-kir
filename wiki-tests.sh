...
if [ "$4" == "update" ]; then
    ...
    wget -O $TMPHTML -q http://wiki.apertium.org/wiki/Turkish_and_Kyrgyz/$TESTTYPE
    if [[ -s $TMPHTML ]]; then mv $TMPHTML $HTML;
    else rm $TMPHTML; echo "Couldn't fetch http://wiki.apertium.org/wiki/Turkish_and_Kyrgyz/$TESTTYPE"; fi
fi
echo "..."
